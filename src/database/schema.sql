...
CREATE POLICY users_select_policy ON users
  FOR SELECT USING (
    -- Admins and orientation can see all users
    auth.uid() IN (SELECT id FROM users WHERE role IN ('admin', 'orientation'))
    -- Parents can see themselves and their children
    OR auth.uid() IN (SELECT id FROM users WHERE role = 'parent' AND id = users.id)
    -- Professors can see themselves
    OR auth.uid() IN (SELECT id FROM users WHERE role = 'professor' AND id = users.id)
  );
